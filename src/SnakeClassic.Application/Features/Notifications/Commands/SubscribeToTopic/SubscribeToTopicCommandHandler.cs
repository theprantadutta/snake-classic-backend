...
public class SubscribeToTopicCommandHandler
    : IRequestHandler<SubscribeToTopicCommand, Result<SubscribeTopicResultDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await _messaging.SubscribeToTopicAsync(request.Token, request.Topic);

            // Update token's subscribed topics
            var fcmToken = await _context.FcmTokens
                .FirstOrDefaultAsync(t => t.Token == request.Token, cancellationToken);

            if (fcmToken != null && !fcmToken.SubscribedTopics.Contains(request.Topic))
            {
                fcmToken.SubscribedTopics.Add(request.Topic);
