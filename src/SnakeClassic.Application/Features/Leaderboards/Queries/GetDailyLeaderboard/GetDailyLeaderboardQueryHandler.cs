...
public class GetDailyLeaderboardQueryHandler : IRequestHandler<GetDailyLeaderboardQuery, Result<LeaderboardResponseDto>>
{
    private readonly IAppDbContext _context;
    private readonly ICurrentUserService _currentUser;
    private readonly IDateTimeService _dateTime;

    public GetDailyLeaderboardQueryHandler(
    ...
    public async Task<Result<LeaderboardResponseDto>> Handle(GetDailyLeaderboardQuery request, CancellationToken cancellationToken)
    {
        var startOfDay = _dateTime.UtcNow.Date;

        var dailyScores = await _context.Scores
            .AsNoTracking()
            .Where(s => s.CreatedAt >= startOfDay)
            .GroupBy(s => s.UserId)
            .Select(g => new
            {
                UserId = g.Key,
        ...
        var userIds = dailyScores.Select(x => x.UserId).ToList();
        var users = await _context.Users
            .AsNoTracking()
            .Where(u => userIds.Contains(u.Id))
            .ToDictionaryAsync(u => u.Id, cancellationToken);

        var entries = dailyScores.Select((x, index) => new LeaderboardEntryDto(
        ...
        if (_currentUser.IsAuthenticated && _currentUser.UserId.HasValue)
        {
            var userBestScore = await _context.Scores
                .Where(s => s.UserId == _currentUser.UserId.Value && s.CreatedAt >= startOfDay)
